...
<script type="text/javascript">
    ...
    $(function () {
        $freshLogin = localStorage.getItem('freshLogin');
                    ...
                    tour.addStep('step5', {
                        ...
                        buttons: [
                            ...
                            },
                            {
                                ...
                                classes: 'btn btn-primary',
                                action: function(){
                                    if($('tbody .redirect:eq(0)').parent().hasClass('shown') == false){
            ...
            $('.toggleInfo').unbind().click(function () {
                ...
                if (row.child.isShown()) {
                    ...
                    setTimeout(function () {
                        ...
                        $('.shareBtn').unbind().click(function() {
                            $btn = $(this);
...
//        $('#date-range-picker').data('daterangepicker').setEndDate(moment($searchHistory.endDate).format('DD-MM-Y HH:mm:ss'));
    ...
    function format(d) {
        $template = '<div class="row card-slider">' +
            ...
            '<button data-id="' + d + '" class="btn btn-default pull-right star mrg-right-10" id="firstStar" title="Save"><i id="star-' + d + '" class="star-icon ion-ios7-star-outline font-size-20"></i></button>' +
            '</div>' +
    ...
    function saveClickedDas(clickedDas){
        $.ajax({
            url: '{{ url('search/saveClickedDas') }}',
            type: 'POST',
            data: {
                clickedDas: clickedDas
            },
